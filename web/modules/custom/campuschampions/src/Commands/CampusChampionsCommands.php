...
use Drupal\campuschampions\Plugin\StoreStats;
...
class CampusChampionsCommands extends DrushCommands {
   ...
   * The stats service.
   *
   * @var \Drupal\campuschampions\Plugin\StoreStats
   */
  ...
  /**
   * Constructs a CampusChampionsCommands object.
   *
   ...
   *   The request stack.
   * @param \Drupal\campuschampions\Plugin\StoreStats $stats_service
   *   The stats service.
  ...
  public function exportAffinityGroups() {
    ...
    foreach ($nodes as $affinityGroup) {
      ...
      $view->setArguments([$affinityGroup->id()]);
      $view->execute();
      $rendered = $view->render();
      $output = $this->renderer->renderInIsolation($rendered);

      $filename = 'affinity-groups/affinity_group_' . $affinityGroup->getTitle() . '_' . date('Y_m_d') . '.csv';
      $filepath = "sites/default/files/" . $filename;
      $this->fileRepository->writeData($output, "public://{$filename}", FileExists::Replace);
...
';
   ...
   */
  public function generateBreakdownStats($options = []) {
    ...
    $this->state->set('cc_stats', json_encode($data));

    $result = $this->fileRepository->writeData(
      json_encode($data),
      $path,
      FileExists::Replace
    );
    ...
    $realpath = $this->fileSystem->realpath($dir) . '/' . $file;
  ...
  public function updateStats($options = []) {
    ...
    $this->statsService->setState();
    $this->state->set('cc_stats_last_update', $this->time->getCurrentTime());

    $stats = json_decode($this->state->get('cc_stats'), TRUE);
  ...
  public function identifyEduMissingCarnegie($options = ['export' => FALSE, 'all-users' => FALSE]) {
    ...
    if (!$options['all-users']) {
      $query->condition('field_region.target_id', 572);
    }
    ...
    $with_institution = array_filter($users_data, function ($u) {
      return !empty($u['institution']);
    });
    $this->output()->writeln(sprintf("- Users with institution field populated: %d", count($with_institution)));

    $with_access_org = array_filter($users_data, function ($u) {
      return !empty($u['access_org']);
    });
    $this->output()->writeln(sprintf("- Users with ACCESS Organization: %d", count($with_access_org)));

    $with_invalid = array_filter($users_data, function ($u) {
      return strpos($u['current_carnegie'], 'INVALID') !== FALSE;
    });
      ...
      foreach ($users as $user) {
        ...
        if (!$options['organizations-only']) {
          ...
          if ($needs_carnegie_update) {
            ...
            if (!empty($access_org)) {
              $org_node = Node::load($access_org[0]['target_id']);
              if ($org_node) {
  ...
  protected function findCarnegieCode(User $user, $auto_approve = FALSE, $access_org_name = '') {
    ...
    if ($user->hasField('field_access_organization')) {
      ...
      if (!empty($org)) {
        ...
        $match_result = $this->findAccessOrganization($user, TRUE);
    ...
    if (!empty($institution_name)) {
      ...
      static $user_decisions_carnegie = [];
      ...
      if (!isset($carnegie_cache[$cache_key])) {
        ...
        $query = $this->database
          ...
          ->fields('cc', ['unitid', 'instnm']);
        $query->condition('instnm', $institution_name, '=');
        ...
        if ($exact_result) {
          ...
          $like_query = $this->database
            ->select('carnegie_codes', 'cc')
            ->fields('cc', ['unitid', 'instnm']);
          $like_query->condition('instnm', '%' . $institution_name . '%', 'LIKE');
          $like_query->range(0, 10);
          ...
          if (!empty($partial_results)) {
            if ($auto_approve) {
              ...
              if (!isset($user_decisions_carnegie[$decision_key])) {
                ...
                foreach ($partial_results as $i => $result) {
                  $this->output()->writeln(sprintf("  [%d] %s (ID: %s)", $i + 1, $result->instnm, $result->unitid));
                  $options_list[$i + 1] = $result->unitid;
                }
                ...
                if ($choice === 's' || $choice === 'S') {
                  ...
                  $user_decisions_carnegie[$decision_key] = $search_result;
                }
                elseif ($choice > 0 && isset($options_list[$choice])) {
                  $user_decisions_carnegie[$decision_key] = $options_list[$choice];
                }
                else {
                  $user_decisions_carnegie[$decision_key] = NULL;
                }
              }

              $carnegie_cache[$cache_key] = $user_decisions_carnegie[$decision_key];
            }
          }
          else {
    ...
    if (!$auto_approve) {
      ...
      if (!isset($user_decisions_no_inst[$decision_key])) {
        ...
        if ($search) {
          ...
          $user_decisions_no_inst[$decision_key] = $search_result;
          return $search_result;
        }
        else {
          $user_decisions_no_inst[$decision_key] = NULL;
        }
      }

      return $user_decisions_no_inst[$decision_key];
    }
      ...
      if (!$org_node) {
        ...
        if (!empty($partial_matches)) {
          if ($auto_approve) {
            ...
            if (!isset($user_decisions_org[$decision_key])) {
              $this->output()->writeln(sprintf("\nUser institution: '%s'", $institution_name));
              ...
              if ($choice === 's' || $choice === 'S') {
                ...
                if ($search_result) {
                  $user_decisions_org[$decision_key] = $search_result;
                }
                else {
                  $user_decisions_org[$decision_key] = NULL;
                }
              }
              elseif ($choice > 0 && isset($options_list[$choice])) {
                $user_decisions_org[$decision_key] = $options_list[$choice];
              }
              else {
                $user_decisions_org[$decision_key] = NULL;
              }
            ...
            if ($user_decisions_org[$decision_key]) {
              $org_node = $user_decisions_org[$decision_key]['node'];
              $match_type = $user_decisions_org[$decision_key]['type'] . '-approved';
            }
          }
        }
        else {
          ...
          if (!$auto_approve) {
            ...
            elseif ($user_decisions_org[$decision_key]) {
              $org_node = $user_decisions_org[$decision_key]['node'];
              $match_type = $user_decisions_org[$decision_key]['type'] . '-search';
            }
          }
        }
      }
  ...
  protected function interactiveSearch($institution_name) {
    while (TRUE) {
      ...
      $choice = $this->io()->ask('Select an option (0-' . (count($options_list)) . ', s)', '0');
  ...
  protected function interactiveCarnegieSearch($institution_name, $access_org_name = '') {
    while (TRUE) {
      ...
      $query = $this->database
        ->select('carnegie_codes', 'cc')
        ->fields('cc', ['unitid', 'instnm'])
        ->condition('instnm', '%' . $search_term . '%', 'LIKE')
        ->range(0, 20);
      ...
      foreach ($results as $result) {
        $this->output()->writeln(sprintf("  [%d] %s (ID: %s)", $i, $result->instnm, $result->unitid));
        $options_list[$i] = $result->unitid;
        $i++;
      }
