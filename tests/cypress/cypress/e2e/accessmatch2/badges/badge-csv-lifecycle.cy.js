/*
 * Badge CSV Assignment — Full Lifecycle Test (badges vocabulary / MATCH)
 *
 * Domain: accessmatch.ddev.site
 * Vocabulary: badges
 * Badge term: MATCH
 *
...
const VOCABULARY = 'badges';
const BADGE_TERM = 'MATCH';

describe('Badge CSV Lifecycle — badges / MATCH', () => {
  ...
  it('Verify Pecan Pie has the MATCH badge assigned', () => {
    cy.loginAs(ADMIN_USER, ADMIN_PASS);

    // Check the badge was assigned via ddev drush SQL query.
    cy.exec(
      `ddev drush sqlq "SELECT COUNT(*) FROM user__field_user_badges ub INNER JOIN users_field_data ufd ON ub.entity_id = ufd.uid INNER JOIN taxonomy_term_field_data ttfd ON ub.field_user_badges_target_id = ttfd.tid WHERE ufd.mail = 'pecan@pie.org' AND ttfd.name = '${BADGE_TERM}'"`,
...
/**
 * Removes the MATCH badge from Pecan Pie if it was assigned during the test.
 */
function cleanupAssignedBadges() {
  cy.exec(
    `ddev drush sqlq "DELETE ub FROM user__field_user_badges ub INNER JOIN users_field_data ufd ON ub.entity_id = ufd.uid INNER JOIN taxonomy_term_field_data ttfd ON ub.field_user_badges_target_id = ttfd.tid WHERE ufd.mail = 'pecan@pie.org' AND ttfd.name = '${BADGE_TERM}'"`,
    { failOnNonZeroExit: false, timeout: 30000 }
  ).then((result) => {
